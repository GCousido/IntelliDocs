const inferredApiBase = () => {
    ...
    return "http://172.16.167.135:31001";
};

const API_BASES = [
    ...new Set(
        [window.API_BASE, inferredApiBase(), "http://172.16.167.135:31001"]
            .filter(Boolean)
...
function renderPipelineExample() {
    ...
    if (!doc) {
        pipelinePreview.innerHTML =
            '<div class="document-card">Todavía no hay documentos procesados para mostrar un ejemplo completo.</div>';
        return;
    }
...
async function fetchDocuments() {
    try {
        ...
        state.documents = [];
        listEl.innerHTML =
            '<div class="document-card">No se pudo cargar la biblioteca de documentos.</div>';
...
function renderDocuments() {
    ...
    if (!state.documents.length) {
        listEl.innerHTML =
            '<div class="document-card">Aún no hay documentos procesados.</div>';
        return;
    }
